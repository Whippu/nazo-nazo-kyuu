...
class TextEditor(object):
    ...
    def __init__(self, path):
        ...
        self.bytes = None
        self.pointers = None
        self.partitioned_bytes = None
        self.table = self.loader.table

    # def search_all_files_for_phrase(self, phrase):
    #     successes = []
    #     for file_name in self.loader.files:
    #         try:
    #             self.load_text(file_name)
    #             if self.find_phrase(phrase) is not None:
    ...
    """
    def refresh(self):
        self.unpack_file(self.current_file)

    def do_everything(self, file_name):
        self.unpack_file(file_name)
        self.run_replacement()
        self.reconstruct_bytes()

    def refresh_everything(self):
        self.do_everything(self.current_file)

    def get_translations(self, file_name=None):
        if file_name is None:
            file_name = self.current_file

        self.unpack_file(file_name)
        self.output_text(as_bytes=True)
        self.output_text(as_bytes=False)
    ...
    def unpack_file(self, file_name):
        ...
        pointers = parse_pointer_table(raw_bytes)
        self.pointers = pointers
        self.partitioned_bytes = {}
        for start, end in zip(pointers, pointers[1:]+[None]):
            self.partitioned_bytes[start] = raw_bytes[start:end]

    def bytes_to_text(self, raw_bytes, replace_unknown=True, as_bytes=False):

        new_text_chars = []
        counter = 0
        text_len_counter = 0
        byte_correspondence_dict = {}
        control_object = Chikuhouse(raw_bytes, self.table)

        while counter < len(raw_bytes):
            counter_increment = 1
            byte = raw_bytes[counter]
            ...
            byte_correspondence_dict[text_len_counter] = (raw_bytes[counter:counter + counter_increment], text_to_append)
    ...
    def run_replacement(self):
        ...
        replacement_file = os.path.join(self.loader.path, self.loader.DATA_FOLDER, self.current_file + '.rpl')
        with open(replacement_file, 'r', encoding='utf-8') as fh:
            lines = fh.readlines()


        current_pointer = None
        text, correspondence_dict = None, None
        ...
        for line in lines:
            ...
            if line.startswith('!'):
                current_pointer = int(line[1:])
                text, correspondence_dict = self.bytes_to_text(self.partitioned_bytes[current_pointer])
            ...
            needle, replacement = [x.strip() for x in line.strip().split('|')]
            ...
            corresponding_bytes = self.find_corresponding_bytes(correspondence_dict, *position)
            start = self.partitioned_bytes[current_pointer].find(corresponding_bytes)
            assert start > -1
            end = start + len(corresponding_bytes)

            new_bytes = self.translate_phrase(replacement)

            self.partitioned_bytes[current_pointer] = self.partitioned_bytes[current_pointer][0:start] + new_bytes + \
                                                      self.partitioned_bytes[current_pointer][end:]
    ...
    def output_text(self, as_bytes=False):
        output_text = ''
        for pointer in self.pointers:
            output_text += f'Pointer: {pointer}\n'
            raw_bytes = self.partitioned_bytes[pointer]
            text, correspondence_dict = self.bytes_to_text(raw_bytes, as_bytes=as_bytes)
        ...
        file_name = os.path.join(self.loader.path, self.loader.DATA_FOLDER, self.current_file + suffix)
    ...
    def reconstruct_bytes(self, output=True):

        new_pointers = []

        payload_offset = self.pointers[0]
        payload = bytes()

        for pointer in self.pointers:
            bytes_to_append = self.partitioned_bytes[pointer]
            corresponding_pointer = payload_offset + len(payload)

            payload += bytes_to_append
            new_pointers.append(corresponding_pointer)

        pointer_table_bytes = reconstruct_pointer_table(new_pointers)
        bytes_to_output = pointer_table_bytes + payload

        if output:
            file_name = os.path.join(self.loader.path, self.loader.DATA_FOLDER, 'RECONSTRUCTED_' + self.current_file)
            with open(file_name, 'wb') as fh:
                fh.write(bytes_to_output)
            print(f'Output to: {file_name}')
